# -*- coding: utf-8 -*-
"""esr_bsr_gan_with_text_fin

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1PZsvkU8Rz4kQAOYmdELCFpxWOvKdoRKA
...
"""## Download BSRGAN and ESRGAN Model"""
...
def create_degraded_images(image_files, local_input_dir, local_degraded_dir, drive_degraded_dir, scale_factor=0.5):
    """Create degraded (low-resolution) versions of input images without adding noise"""
    degraded_files = []
    ...
    for filename in image_files:
        ...
        # 2. Apply JPEG compression artifacts - используем более агрессивное сжатие
        encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 50]  # Низкое качество JPEG
        _, buffer = cv2.imencode('.jpg', img_lr, encode_param)
        img_lr = cv2.imdecode(buffer, cv2.IMREAD_COLOR)

        # 3. Применить легкое размытие для имитации отсутствия четкого фокуса
...
def process_image(degraded_path, output_path, model, device, original_path=None, tile_size=None):
    ...
    # Сохраняем цвета из оригинального изображения, если оно есть
    if original_path is not None:
        try:
            # Загружаем оригинальное изображение
            original = cv2.imread(original_path)
            if original is not None:
                # Изменяем размер оригинала до размера улучшенного изображения
            ...
            # Продолжаем без сохранения цвета
...
def process_all_images(image_files, model, device, local_input_dir, local_degraded_dir, local_output_dir, drive_output_dir, model_name="BSRGAN", tile_size=None):
    """Process all images with the model - displays a clear before/after comparison"""
    all_metrics = []
    ...
    for filename in image_files:
        ...
        try:
            # Process image - capture the metrics - теперь передаем original_path
            ...
            ocr_accuracy = calculate_ocr_accuracy(input_path, output_path)
...
def compare_model_results(all_results, model1="BSRGAN", model2="ESRGAN"):
    ...
    # Calculate averages
    metrics_to_average = ['psnr', 'ssim', 'sharpness_increase', 'detail_increase', 'ocr_accuracy']
    ...
    metrics_to_plot = ['psnr', 'ssim', 'ocr_accuracy']
    labels = ['PSNR (dB)', 'SSIM (x100)', 'OCR Accuracy (%)']

    model1_values = [model1_avg['psnr'], model1_avg['ssim']*100, model1_avg['ocr_accuracy']]
    model2_values = [model2_avg['psnr'], model2_avg['ssim']*100, model2_avg['ocr_accuracy']]
...
def main():
    ...
    # Set up directories - CHANGE THESE PATHS to match your Google Drive structure
    drive_input_dir = '/content/drive/MyDrive/images_with_text'
    drive_output_dir = '/content/drive/MyDrive/gan_output_images'
    drive_degraded_dir = '/content/drive/MyDrive/gan_degraded_images'
    ...
    # Download pre-trained BSRGAN model
    os.system('wget -nc https://github.com/cszn/KAIR/releases/download/v1.0/BSRGAN.pth -P /content/models/')

    # Download pre-trained ESRGAN model
    os.system('gdown --id 1TPrz5QKd8DHHt1k8SRtm6tMiPjz_Qene -O /content/models/RRDB_ESRGAN_x4.pth')
    ...
    if not image_files:
        ...
        # Create degraded versions with more visible degradation
    print("\n=== Creating visibly degraded versions of images ===")
    ...
    bsrgan_results = process_all_images(
        ...
        model_name="BSRGAN", tile_size=512
    )
    ...
    esrgan_results = process_all_images(
        ...
        local_input_dir, local_degraded_dir, local_output_dir, drive_output_dir,
        model_name="ESRGAN", tile_size=512
    )
